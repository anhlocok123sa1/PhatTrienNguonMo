...
<table>
	...
	$sql = "SELECT * FROM nguoidung LIMIT :from, :max_results";
	$query = $dbh->prepare($sql);
	$query->bindParam(':from', $from, PDO::PARAM_INT);
	$query->bindParam(':max_results', $max_results, PDO::PARAM_INT);
	$query->execute();
	...
	if ($dem > 0)
		while ($bien = $query->fetch(PDO::FETCH_ASSOC)) {
			...
			<tr class='noidung_hienthi_sp'>
				...
				<td class="stt_hienthi_sp">
					<?php echo $bien['tennd'] ?>
				</td>
				<td class="img_hienthi_sp">
					<?php echo $bien['username'] ?>
				</td>
				<td class="sl_hienthi_sp">
					<?php echo $bien['email'] ?>
				</td>
				<td class="sl_hienthi_sp">0
					<?php echo $bien['dienthoai'] ?>
				</td>
				<td class="sl_hienthi_sp">
					<?php
					if ($bien['phanquyen'] == 0)
...
<div id="phantrang_sp">
	...
	$sql = "SELECT COUNT(*) as Num FROM nguoidung";
	$query = $dbh->query($sql);
