...
<table>
	...
	/*------------Phan trang------------- */
		...
		$sql = $dbh->query("SELECT hoadon.*, tensp FROM hoadon INNER JOIN chitiethoadon WHERE hoadon.mahd = chitiethoadon.mahd ORDER by mahd DESC  LIMIT $from, $max_results"); 
		

								
    if($dem > 0)
        while ($bien = $sql->fetch(PDO::FETCH_ASSOC))
        {
?>
...
?>
	...
	<?php
			...
			$query = $dbh->query("SELECT COUNT(*) as Num FROM hoadon");
